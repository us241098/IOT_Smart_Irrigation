...
while True:
    if (GPIO.input(INPUT_PIN_1) == True): # Physically read the pin now
        publish.single(MQTT_PATH, "2-1", hostname=MQTT_SERVER)
    else:
        publish.single(MQTT_PATH, "2-0", hostname=MQTT_SERVER)

    if (GPIO.input(INPUT_PIN_2) == True): # Physically read the pin now
        publish.single(MQTT_PATH, "3-1", hostname=MQTT_SERVER)
    else:
        publish.single(MQTT_PATH, "3-0", hostname=MQTT_SERVER)
